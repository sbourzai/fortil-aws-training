# Security group
resource "aws_security_group" "training_sg_for_elb" {
  name   = "${var.env_prefix}-sg_for_elb"
...
resource "aws_security_group" "training_sg_for_ec2" {
  name   = "${var.env_prefix}-sg_for_ec2"
  vpc_id = aws_vpc.vpc_main.id
